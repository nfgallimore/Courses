...
void draw(void)
{     
    int x = 0;                                              // x is horizontal in the two dimensional array
    int y = 0;                                              // and y is the vertical array in the data set
    int max = DIMENSION;                                    // DIMENSION can get annoying to type
    int xTemp, yTemp;                                       // temp placeholders for both x and y (solution to '\0')
    bool tempUsed, nullChar;                                // tempUsed returns true temp != x or y. nullChar returns true if '\0' is printed.    
    int gridtemp = { grid[x][y] };                          // sets temporary array equal to original grid dataset    
    printf("\n");                                           // needs a line after the word score
    
  /*********************************************************** NOW FOR THE IMPORTANT STUFF **********************/
                                                            
    if (grid[x][y] != '\0') nullChar = false;              // sets nullChar to false, continues in both for loops
    {                                             
        for (x = 0; x <= DIMENSION , nullChar = false; x++)          // print row 0 first until x = max
        {
            for (y = 0; (x == max) && (y <= max); y++)          // increments y, only if x = max, 
            {                                      
                if (y == max)                                  // when both y and x hit max, its the end of array
                printf("\n"), x = 0;                           // new line, sets x to 0                                                             
            }        
            printf("%c" , grid[x][y]);
        }
    }
    ...
    if ( nullChar == true)          
        {
            // temporary x and y placeholder s, also sets tmp to true since 
            // we know the new grid[x][y] values aren't iterations
            xTemp = x, yTemp = y;
            tempUsed = true;
            
            // randomization of temporary x and y variables
            xTemp = y % x;
            yTemp = x % y; 
            
            // prints gridtemp[xTemp][yTemp]
            printf("%c", grid[xTemp][yTemp]);
            
            // sets temp x and y to correct values for future uses
            xTemp = x, yTemp = y;
            tempUsed = false;
            
            // make sure nullChar is FALSE or it will mess up future iterations of x and y
            if (tempUsed == true && nullChar == false)
            
                // prints something other than the nullChar
                printf("%c", grid[xTemp][yTemp]);
       }
